...
PBRBaker::PBRBaker(Context &ctx) :
    ctx_(ctx),
    desc_state_(ctx_.device)
{
...
void PBRBaker::load_background()
{
	...
	ImageResource     resource  = ImageResource::create_empty_cubic_img_resrc(ctx_.device, img_tinfo.meta);

	CommandBuffer cmd_buf     = ctx_.device.begin_one_time_buf();
	Buffer        staging_buf = ctx_.device.get_device_memory_allocator().allocate_staging_buffer(img_tinfo.binary.size());
	...
	vk::SamplerCreateInfo sampler_cinfo = Sampler::linear_clamp_cinfo(ctx_.physical_device, img_tinfo.meta.levels);

	result_.p_background = std::make_unique<Texture>(std::move(resource), Sampler(ctx_.device, sampler_cinfo));
}
...
void PBRBaker::bake_irradiance(ImageMetaInfo &cube_meta)
{
	...
	CommandBuffer     bake_buf        = ctx_.device.begin_one_time_buf();
...
void PBRBaker::create_brdf_lut_texture()
{
	...
	ImageView view = ImageView(ctx_.device, view_cinfo);

	vk::SamplerCreateInfo sample_cinfo = Sampler::linear_clamp_cinfo(ctx_.physical_device, 1);

	result_.p_brdf_lut = std::make_unique<Texture>(
	    ImageResource(std::move(img), std::move(view)),
	    Sampler(ctx_.device, sample_cinfo));
}
...
void PBRBaker::bake_brdf_lut()
{
	...
	GraphicsPipeline pl = GraphicsPipeline(ctx_.device, render_pass, pl_state, pl_layout_cinfo);
...
{
	vk::SamplerCreateInfo sampler_cinfo = Sampler::linear_clamp_cinfo(ctx_.physical_device, cube_meta.levels);

	return std::make_unique<Texture>(create_empty_cubic_img_resource(cube_meta), Sampler(ctx_.device, sampler_cinfo));
}
...
ImageResource PBRBaker::create_empty_cubic_img_resource(ImageMetaInfo &meta)
{
	...
	Image img = ctx_.device.get_device_memory_allocator().allocate_device_only_image(img_cinfo);
	...
	return ImageResource(std::move(img), ImageView(ctx_.device, view_cinfo));
}
...
RenderPass PBRBaker::create_color_only_renderpass(vk::Format format, vk::ImageLayout initial_layout, vk::ImageLayout final_layout)
{
	...
	return RenderPass(ctx_.device, render_pass_cinfo);
}
...
ImageResource PBRBaker::create_transfer_src(vk::Extent3D extent, vk::Format format)
{
	...
	Image img = ctx_.device.get_device_memory_allocator().allocate_device_only_image(transfer_src_cinfo);

	vk::ImageViewCreateInfo view_cinfo = ImageView::two_dim_view_cinfo(img.get_handle(), format, vk::ImageAspectFlagBits::eColor, 1);

	ImageResource resrc = ImageResource(std::move(img), ImageView(ctx_.device, view_cinfo));
...
Framebuffer PBRBaker::create_square_framebuffer(const RenderPass &render_pass, const ImageView &view, uint32_t dimension)
{
	...
	return Framebuffer(ctx_.device, framebuffer_cinfo);
}
...
GraphicsPipeline PBRBaker::create_graphics_pipeline(RenderPass &render_pass, vk::PipelineLayoutCreateInfo &pl_layout_cinfo, const char *vert_shader_name, const char *frag_shader_name)
{
	...
	return GraphicsPipeline(ctx_.device, render_pass, state, pl_layout_cinfo);
}
